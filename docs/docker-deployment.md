...
- `migrate`：使用独立 DDL 账号执行 V001–V008，成功退出后才允许 API 启动。
...
正式运行镜像不包含测试、`.env`、数据库备份、外部开发模拟器程序、截图、本地依赖目录或压测/演示脚本；API 镜像只保留迁移入口和 V001–V008 SQL。应用内仍保留被禁用的模拟器控制边界，以支持高低温湿热二室安装和夹具就绪的 hostless 例外。
