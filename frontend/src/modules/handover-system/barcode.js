const CODE128_PATTERNS = [
  "212222", "222122", "222221", "121223", "121322", "131222", "122213", "122312", "132212", "221213", "221312",
  "231212", "112232", "122132", "122231", "113222", "123122", "123221", "223211", "221132", "221231", "213212",
  "223112", "312131", "311222", "321122", "321221", "312212", "322112", "322211", "212123", "212321", "232121",
  "111323", "131123", "131321", "112313", "132113", "132311", "211313", "231113", "231311", "112133", "112331",
  "132131", "113123", "113321", "133121", "313121", "211331", "231131", "213113", "213311", "213131", "311123",
  "311321", "331121", "312113", "312311", "332111", "314111", "221411", "431111", "111224", "111422", "121124",
  "121421", "141122", "141221", "112214", "112412", "122114", "122411", "142112", "142211", "241211", "221114",
  "413111", "241112", "134111", "111242", "121142", "121241", "114212", "124112", "124211", "411212", "421112",
  "421211", "212141", "214121", "412121", "111143", "111341", "131141", "114113", "114311", "411113", "411311",
  "113141", "114131", "311141", "411131", "211412", "211214", "211232", "2331112",
];

const XML_ESCAPE_MAP = {
  "&": "&amp;",
  "\"": "&quot;",
  "<": "&lt;",
  ">": "&gt;",
};

const encodeXml = (value) => String(value || "").replace(/[&"<>]/g, (char) => XML_ESCAPE_MAP[char] || char);

const normalizeCode128Text = (value) => String(value || "").replace(/[^\x20-\x7E]/g, "-");

const encodeCode128B = (value) => {
  const text = normalizeCode128Text(value);
  const codes = [104];
  for (let index = 0; index < text.length; index += 1) {
    codes.push(text.charCodeAt(index) - 32);
  }
  let checksum = 104;
  for (let index = 1; index < codes.length; index += 1) {
    checksum += codes[index] * index;
  }
  checksum %= 103;
  codes.push(checksum, 106);
  return { text, codes };
};

export const buildCode128Svg = (value, options = {}) => {
  const encoded = encodeCode128B(value);
  const moduleWidth = options.moduleWidth || 2;
  const quietZone = options.quietZone || 10;
  const height = options.height || 90;
  let totalModules = quietZone * 2;

  encoded.codes.forEach((itemCode) => {
    const pattern = CODE128_PATTERNS[itemCode] || "";
    for (let index = 0; index < pattern.length; index += 1) {
      totalModules += Number.parseInt(pattern[index], 10) || 0;
    }
  });

  const width = totalModules * moduleWidth;
  let cursor = quietZone * moduleWidth;
  const rects = encoded.codes.map((itemCode) => {
    const pattern = CODE128_PATTERNS[itemCode] || "";
    let rectMarkup = "";
    for (let index = 0; index < pattern.length; index += 1) {
      const unit = Number.parseInt(pattern[index], 10) || 0;
      const barWidth = unit * moduleWidth;
      if (index % 2 === 0 && barWidth > 0) {
        rectMarkup += `<rect x="${cursor}" y="0" width="${barWidth}" height="${height}" fill="#000" />`;
      }
      cursor += barWidth;
    }
    return rectMarkup;
  }).join("");

  return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}" aria-label="${encodeXml(encoded.text)}">${rects}</svg>`;
};

export { normalizeCode128Text };
