...
describe("visualization styles", () => {
  ...
  test("layout A keeps fixed vertical budget so content does not collapse", () => {
    ...
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-lab-panel\s*{[^}]*grid-template-columns:\s*minmax\(230px,\s*0\.2fr\) minmax\(0,\s*1fr\);[^}]*grid-template-rows:\s*minmax\(32px,\s*auto\) minmax\(0,\s*1fr\);/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-lab-switchboard\s*{[^}]*grid-column:\s*1;[^}]*grid-row:\s*2;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-tray-flow-list\s*{[^}]*grid-column:\s*2;[^}]*grid-row:\s*2;/s);
    expect(pageSource).not.toContain('class: "visual-lab-status-row"');
    expect(pageSource).not.toContain('class: "visual-ok-strip"');
    expect(pageSource).not.toContain('class: "visual-alert-strip"');
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-line\s*{[^}]*grid-template-columns:\s*repeat\(6,\s*minmax\(0,\s*1fr\)\);[^}]*grid-template-rows:\s*repeat\(3,\s*minmax\(78px,\s*1fr\)\);/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(7\)\s*{[^}]*grid-column:\s*6;[^}]*grid-row:\s*2;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(12\)\s*{[^}]*grid-column:\s*1;[^}]*grid-row:\s*2;/s);
    expect(source).not.toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(\d+\)::before/);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-forward::before\s*{[^}]*left:\s*calc\(-50% - 8px\);[^}]*height:\s*2px;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-backward::before\s*{[^}]*right:\s*calc\(-50% - 8px\);[^}]*height:\s*2px;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-turn::before\s*{[^}]*left:\s*50%;[^}]*width:\s*2px;[^}]*height:\s*calc\(100% \+ 14px\);/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\s*{[^}]*min-height:\s*78px;[^}]*padding:\s*5px 10px;[^}]*border:\s*0;[^}]*background:\s*transparent;/s);
  ...
  test("layout A connects vertical flow turns across stretched rows and enlarges step labels", () => {
    ...
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-turn::before\s*{[^}]*top:\s*calc\(-100% - 14px \+ 19px\);[^}]*height:\s*calc\(100% \+ 14px\);/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-dot\s*{[^}]*width:\s*36px;[^}]*height:\s*36px;[^}]*border-width:\s*3px;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-active \.visual-flow-dot\s*{[^}]*width:\s*40px;[^}]*height:\s*40px;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step strong\s*{[^}]*font-size:\s*20px;/s);
  ...
  test("layout A connects manufacturer return below post-experiment staging for long flows", () => {
    ...
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(13\)\s*{[^}]*grid-column:\s*1;[^}]*grid-row:\s*3;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(14\)\s*{[^}]*grid-column:\s*2;[^}]*grid-row:\s*3;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(15\)\s*{[^}]*grid-column:\s*3;[^}]*grid-row:\s*3;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step:nth-child\(16\)\s*{[^}]*grid-column:\s*4;[^}]*grid-row:\s*3;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-backward::before\s*{[^}]*right:\s*calc\(-50% - 8px\);[^}]*height:\s*2px;/s);
  });
  ...
  test("layout A uses connector direction classes so wrapped row starts do not draw stray horizontal lines", () => {
    ...
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-forward::before\s*{[^}]*left:\s*calc\(-50% - 8px\);[^}]*width:\s*calc\(100% \+ 16px\);[^}]*height:\s*2px;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-backward::before\s*{[^}]*right:\s*calc\(-50% - 8px\);[^}]*left:\s*auto;[^}]*width:\s*calc\(100% \+ 16px\);[^}]*height:\s*2px;/s);
    expect(source).toMatch(/\.visual-board\.is-layout-a \.visual-flow-step\.is-connector-turn::before\s*{[^}]*left:\s*50%;[^}]*width:\s*2px;[^}]*height:\s*calc\(100% \+ 14px\);/s);
  ...
  test("today task plan screen enlarges real-data table text and fills cells", () => {
    ...
    expect(source).toMatch(/\.visual-task-plan-row strong\s*{[^}]*width:\s*100%;[^}]*font-size:\s*20px;/s);
    expect(source).toMatch(/\.visual-task-plan-row span\s*{[^}]*width:\s*100%;[^}]*font-size:\s*19px;/s);
