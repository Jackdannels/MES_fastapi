...
上位机每 1 秒为每个试验间发送一条完整状态快照，QoS 为 1：
...
MES 使用服务端接收时间判断新鲜度：默认 5 秒无更新显示“数据延迟”，15 秒显示“上位机失联”。设备分别上报 `online` 与最后成功采集时间 `observed_at`；失联显示“—”和最后有效值，上位机失联时下游状态未知。恢复须连续两次新采样，重复消息不计入恢复。仅新鲜在线数据触发温度超过 60 °C 或电压低于 100 V 的告警，断线前告警保留为待确认。模拟器随机事件和监控链路状态见 [分层通信状态说明](docs/telemetry-monitoring.md)。当前仅维护最新内存状态，不逐条写数据库。高低温湿热二室 `carrier_device.configured=false`，仅安装样品/夹具就绪保留本地模拟，准备、开始和结束仍走 MQTT。
