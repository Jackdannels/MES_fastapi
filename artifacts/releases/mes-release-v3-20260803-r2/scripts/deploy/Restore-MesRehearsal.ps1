...
if (-not $dumpFile -or [System.IO.Path]::GetFileName($dumpFile) -ne $dumpFile) {
    throw "Backup dump file must be a single file name."
}
$dumpPath = Join-Path $backup $dumpFile
...
$helper = (Resolve-Path -LiteralPath (Join-Path $PSScriptRoot "mysql-backup-restore.sh")).Path
$commonArguments = @(
    "run", "--rm",
    "--mount", "type=bind,source=$password,target=/run/secrets/mysql_password,readonly",
    "--mount", "type=bind,source=$helper,target=/opt/mes/mysql-backup-restore.sh,readonly",
...
& docker @commonArguments $ClientImage sh /opt/mes/mysql-backup-restore.sh prepare
if ($LASTEXITCODE -ne 0) { throw "Target database is not empty or could not be prepared." }

& docker @commonArguments `
    --mount "type=bind,source=$dumpPath,target=/backup/database.sql,readonly" `
    $ClientImage `
    sh /opt/mes/mysql-backup-restore.sh restore
