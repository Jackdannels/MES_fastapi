...
function Assert-ArchiveImageContract([string]$ArchivePath, [string[]]$References) {
    ...
    foreach ($reference in $References) {
        ...
        if ($expectedByDigest.ContainsKey($digest)) { throw "Release image digest is duplicated: $digest" }
...
Assert-ArchiveImageContract $archiveEntry[0].full $references
...
foreach ($image in $images) {
    $archiveReference = Get-ArchiveReference ([string]$image.reference)
    $existingTag = Get-ExistingImage $archiveReference
    if ($null -ne $existingTag -and [string]$existingTag.Id -cne [string]$image.image_id) {
        throw "Existing archive tag reference resolves to a conflicting image ID: $archiveReference"
    }
...
foreach ($image in $images) {
    $archiveReference = Get-ArchiveReference ([string]$image.reference)
    $loadedTag = Get-ExistingImage $archiveReference
    if ($null -eq $loadedTag -or [string]$loadedTag.Id -cne [string]$image.image_id) {
        throw "Loaded image tag reference is unavailable or conflicting: $archiveReference"
    }
