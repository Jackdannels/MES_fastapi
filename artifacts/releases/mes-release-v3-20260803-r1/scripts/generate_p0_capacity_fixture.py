...
STAGE4_DATABASE_PATTERN = re.compile(r"^[a-z0-9_]+_stage4_test$")
...
def validate_stage4_target(*, host: str, port: int, database: str) -> None:
    if str(host).strip().lower() != "mysql" or int(port) != 3306:
        raise RuntimeError("Stage4容量夹具只允许连接Compose内部目标 mysql:3306")
    if not STAGE4_DATABASE_PATTERN.fullmatch(str(database or "").strip()):
        raise RuntimeError("Stage4容量夹具数据库必须严格以 _stage4_test 结尾")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--stage4-isolated", action="store_true")
...
def main() -> int:
    ...
    if args.apply:
        ...
        if args.stage4_isolated:
            validate_stage4_target(
                host=settings.MYSQL_HOST,
                port=settings.MYSQL_PORT,
                database=settings.MYSQL_DATABASE,
            )
        initialization = initialize_mysql_storage(seed_demo=False)
        if args.stage4_isolated and int(initialization.get("task_count") or 0) != 0:
            raise SystemExit("Stage4容量夹具拒绝覆盖非空数据库")
