...
def test_mysql_mark_run_ended_keeps_experiment_open_when_bound_tray_is_not_finished(monkeypatch):
    ...
    assert written["mes.experiments"][0]["status"] == "实验进行中"
    assert written["mes.schedules"][0]["status"] == "实验进行中"
    assert written["mes.samples"][0]["location"] == "外观检测间"
    assert written["mes.samples"][0]["status"] == "送至外观检测间"
    assert written["mes.samples"][0]["flow_status"] == "送至外观检测间"
    assert written["mes.samples"][0]["trays"][0]["status"] == "送至外观检测间"
