...
router = APIRouter(prefix="/yt_report", tags=["yt_report"])
store = InMemoryStore()
...
def create_yt_report(payload: EntityCreate):
    return store.create(payload.model_dump())


@router.put("/{item_id}", response_model=EntityOut)
def update_yt_report(item_id: int, payload: EntityUpdate):
...
def delete_yt_report(item_id: int):
