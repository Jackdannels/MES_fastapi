...
from app.services.external_task_intake_service import (
    EXTERNAL_INTAKE_ACCEPTED,
    EXTERNAL_INTAKE_PENDING,
    EXTERNAL_INTAKES_KEY,
    EXTERNAL_SOURCE,
    INTEGRATION_KEYS,
    INTERNAL_SOURCE,
    INVALID_TASK_TEXT_PATTERN,
    LIMS_INBOX_KEY,
    LIMS_OUTBOX_KEY,
    MAX_CONTACT_LENGTH,
    MAX_SAMPLE_COUNT,
    ...
    build_lims_status_event,
    ensure_unique_external_intake,
    ensure_unique_task_code,
    external_intake_id,
    external_intake_status,
    find_external_intake_index,
    load_snapshot as load_external_task_snapshot,
    normalize_text,
    parse_test_types,
    task_code,
...
def list_external_task_intakes(
    ...
    return external_task_intake_service.list_external_task_intakes(
        get_storage_backend(),
        acceptance_status,
    )
...
def store_external_task_intake(payload: dict[str, Any], *, message_id: str = "") -> dict[str, Any]:
    ...
    with EXTERNAL_INTAKE_LOCK:
        return external_task_intake_service.store_external_task_intake(
            get_storage_backend(),
            payload,
            message_id=message_id,
            publish_update=publish_storage_update,
        )
...
def _accept_external_task_intake(intake_id: str) -> dict[str, Any]:
    return external_task_intake_service.accept_external_task_intake(
        get_storage_backend(),
        intake_id,
        add_task_to_snapshot=add_task_to_snapshot,
        publish_update=publish_storage_update,
        task_storage_update_keys=TASK_STORAGE_UPDATE_KEYS,
    )
