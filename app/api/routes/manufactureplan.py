...
router = APIRouter(prefix="/manufactureplan", tags=["manufactureplan"])
store = InMemoryStore()


@router.get("", response_model=list[EntityOut])
def list_manufactureplan():
    return store.list()


@router.get("/{item_id}", response_model=EntityOut)
def get_manufactureplan(item_id: int):
    item = store.get(item_id)
    if item is None:
        raise HTTPException(status_code=404, detail="Manufacture plan not found")
    return item


@router.post("", response_model=EntityOut, status_code=status.HTTP_201_CREATED)
def create_manufactureplan(payload: EntityCreate):
    return store.create(payload.model_dump())


@router.put("/{item_id}", response_model=EntityOut)
def update_manufactureplan(item_id: int, payload: EntityUpdate):
...
def delete_manufactureplan(item_id: int):
