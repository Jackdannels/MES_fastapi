...
def share_files(*, storage: Any, token: str) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    ...
    for record in repository.list_exports(status="success"):
        if _text(record.get("taskCode")) != _text(share.get("taskCode")):
            continue
        if _text(record.get("experimentCode")) != _text(share.get("experimentCode")):
            continue
...
<p>{html.escape(_text(share.get('taskCode')))} / {html.escape(_text(share.get('experimentCode')))}</p>
        ...
        with zipfile.ZipFile(temporary, "w", compression=zipfile.ZIP_DEFLATED) as archive:
            ...
            for item in files:
                ...
                if name in used_names:
                    name = f"{safe_path_segment(item.get('sampleCode'))}-{name}"
                used_names.add(name)
                archive.write(Path(item["filePath"]), arcname=name)
    except Exception:
        temporary.unlink(missing_ok=True)
        raise
    download_name = f"{safe_path_segment(share.get('taskCode'))}-{safe_path_segment(share.get('experimentCode'))}.zip"
